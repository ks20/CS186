...
public class Table implements Iterable<Record>, Closeable {
  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
    ...
    public boolean hasNext() {
      Bits.Bit one = Bits.Bit.ONE;
      for (int check = this.currRid; check < 8 * this.bitMap.length; check++) {
        if (Bits.getBit(this.bitMap, check) == one) {
          return true;
        }
      }
      return false;
    ...
    public RecordId next() {
      Bits.Bit one = Bits.Bit.ONE;
      while (Bits.getBit(this.bitMap, this.currRid) != one) {
        this.currRid = this.currRid + 1;
      }
      short shortCast = (short)(this.currRid);
      this.currRid = this.currRid + 1;
      return new RecordId(this.currPage, shortCast);
    }

    public void mark() {
      this.markRid = (short)(this.currRid - 1);
      assert(this.markRid >= 0);
    }

    public void reset() {
      this.currRid = this.markRid;
    }
  }
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    //private RecordId markedPrevRecordId;

    //private RecordId prevRecordId;
    //private RecordId nextRecordId;

    private BacktrackingIterator<Page> block;
    private BacktrackingIterator<RecordId> blockIter;
    private BacktrackingIterator<RecordId> markedBlockIter;
